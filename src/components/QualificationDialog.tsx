...
import { Theme, QualificationStatutMarital, QualificationSituationPro, QualificationRevenuType, QualificationChargeType, QualificationLiquiditeType, QualificationFrequence } from '../types';
...
interface Revenu {
    ...
    montantBrut: string;
    montantNet: string; // Calculated or overridden
}
...
export const QualificationDialog: React.FC<QualificationDialogProps> = ({ isOpen, onClose, onSave, theme }) => {
    // --- STATE: ETAT CIVIL ---
    ...
    // --- LOGIC: UPDATE REVENU NET ---
    const handleRevenuChange = (id: string, field: keyof Revenu, value: any) => {
        setRevenus(prev => prev.map(r => {
            if (r.id !== id) return r;
            const updated = { ...r, [field]: value };

            // Auto-calculate Net if Brut or Type changes (simplified logic, user can still override Net)
            if (field === 'montantBrut' || field === 'type') {
                const brut = parseFloat(updated.montantBrut) || 0;
                updated.montantNet = calculateNet(brut, updated.type).toFixed(2);
            }
            return updated;
        }));
    };
    ...
    const calculationResults = useMemo(() => {
        ...
        eligibleRevenus.forEach(r => {
            const brut = parseFloat(r.montantBrut) || 0;
            const net = parseFloat(r.montantNet) || 0;
            const amountToUse = r.type === QualificationRevenuType.Dividendes ? brut : net;
            totalRevenusEligibles += annualize(amountToUse, r.frequence);
        });
    ...
    useEffect(() => {
        ...
        // Etat Civil
        if (statutMarital) lines.push(`- Statut: ${statutMarital}`);
        if (declarant1.situationPro) lines.push(`- D1: ${declarant1.situationPro} ${declarant1.anciennete ? `(${declarant1.anciennete} ans)` : ''}`);
        if (showDeclarant2 && declarant2.situationPro) lines.push(`- D2: ${declarant2.situationPro} ${declarant2.anciennete ? `(${declarant2.anciennete} ans)` : ''}`);
        if (enfants.length > 0) lines.push(`- Enfants: ${enfants.length}`);
    ...
    }, [statutMarital, declarant1, declarant2, enfants, calculationResults, showDeclarant2]);
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    ...
                    {isChef && (
                        <div className="space-y-1">
                            ...
                            <Input className="h-8" placeholder="SARL, SAS..." value={declarantState.structureJuridique} onChange={e => setDeclarantState({ ...declarantState, structureJuridique: e.target.value })} />
                        </div>
                    )}
                    {isIndep && (
                        <div className="space-y-1">
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogPortal>
                ...
                <DialogContent className="sm:max-w-[900px] max-h-[90vh] overflow-hidden flex flex-col gap-0 p-0 bg-background z-[100001] pointer-events-auto">
                        ...
                        <div className="space-y-8">
                            ...
                            <section className="space-y-4">
                                ...
                                <div className="space-y-2">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <Button size="sm" variant="outline" onClick={() => setEnfants([...enfants, { id: generateId(), prenom: '', age: '', aCharge: true }])}>
                            ...
                            <section className="space-y-4">
                                <div className="flex items-center justify-between pb-2 border-b">
                                    ...
                                    <div className="flex gap-2">
                                        <Button size="sm" variant="outline" onClick={() => setRevenus([...revenus, { id: generateId(), declarantId: 1, type: '', frequence: QualificationFrequence.Mensuel, montantBrut: '', montantNet: '' }])}>
                                            + Déclarant 1
                                        </Button>
                                        {showDeclarant2 && (
                                            <Button size="sm" variant="outline" onClick={() => setRevenus([...revenus, { id: generateId(), declarantId: 2, type: '', frequence: QualificationFrequence.Mensuel, montantBrut: '', montantNet: '' }])}>
                                                + Déclarant 2
                                            </Button>
                                        )}
                                    </div>
                                        ...
                                        <TableBody>
                                            {revenus.map((rev) => (
                                                <TableRow key={rev.id}>
                                                    ...
                                                    <TableCell>
                                                        <Input type="number" className="h-8" value={rev.montantBrut} onChange={e => handleRevenuChange(rev.id, 'montantBrut', e.target.value)} />
                                                    </TableCell>
                                                    <TableCell>
                                                        <Input type="number" className="h-8 bg-muted/30" value={rev.montantNet} onChange={e => handleRevenuChange(rev.id, 'montantNet', e.target.value)} />
                                                    </TableCell>
                                                    <TableCell>
                            ...
                            <section className="space-y-4">
                                <div className="flex items-center justify-between pb-2 border-b">
                                    ...
                                    <Button size="sm" variant="outline" onClick={() => setLiquidites([...liquidites, { id: generateId(), type: '', montant: '', disponible: true }])}>
